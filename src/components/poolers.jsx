...
const prizeValue = (amount) => {
  let value = parseFloat(amount);
  value = value / 1e14;
  return value.toFixed(0);
};
const prizeValueFloat = (amount) => {
  let value = parseFloat(amount);
  value = value / 1e14;
  return value;
};
const usdcValue = (amount) => {
  let value = parseFloat(amount);
  value = value / 1e6;
  return value;
};
...
async function getPooler(address) {
  try {
    ...
    console.log("failed to fetch api/address");
    return null;
  }
}
...
function processClaimParameters(pooler, claimableWins, network, drawNum) {
  let networkFilter = claimableWins.filter(
    (win) => win.network.toLowerCase() === network.toLowerCase()
  );
  ...
  networkFilter.map((win, index) => {
    ...
    claimData.winningPicks.push(
      win.picks.sort(function (a, b) {
        return a - b;
      })
    );
  });
  claimData.gas = 500000 + (claimData.drawIds.length - 1) * 325000;
  claimData.winningPicks = ethers.utils.defaultAbiCoder.encode(
    ["uint256[][]"],
    [claimData.winningPicks]
  );
...
async function getBalances(address, timestamp) {
  try {
    let [
      ...
      PolygonTicketContract.getBalanceAt(address, timestamp),
      EthereumTicketContract.getBalanceAt(address, timestamp),
      AvaxTicketContract.getBalanceAt(address, timestamp),
      OptimismTicketContract.getBalanceAt(address, timestamp),
    ]);
    let balances = {
...
function processWins(winsYo, claimsYo) {
  ...
  let queryFiltered = notNull.filter(
    (eachWin) => eachWin.claimable_prizes.length > 0
  );
  ...
  queryFiltered.map((win) => {
    ...
    winsArray.push({
      network: capitalizeFirstLetter(win.network),
      draw: win.draw_id,
      win: prizesSum,
      claimed: claimed,
      picks: win.claimable_picks,
    });
...
const walletBalance = (balances, chain) => {
  try {
    ...
    if (chain === "Optimism") {
      balance = balances.optimism;
    }
    if (chain === "Avalanche") {
      balance = balances.avalanche;
    }
    if (chain === "Ethereum") {
...
const ticketBalance = (balances, chain) => {
  try {
    let balance = 0;
    if (chain === "Polygon") {
      balance = balances[0].polygon;
    }
    if (chain === "Optimism") {
      balance = balances[0].optimism;
    }
    if (chain === "Avalanche") {
      balance = balances[0].avalanche;
    }
    if (chain === "Ethereum") {
      balance = balances[0].ethereum;
    }
    return balance;
  } catch (error) {
    console.log(error);
...
function Poolers() {
  ...
  const [prizeDistributor, setPrizeDistributor] = useState(
    "0x722e9BFC008358aC2d445a8d892cF7b62B550F3F"
  ); // starts with OP distributor for no good reason
  ...
  const {
    config: usdcConfig,
    error: usdcConfigError,
    isError: usdcConfigIsError,
  } = usePrepareContractWrite({
    args: [
      prizePoolAddress,
  ...
  const { isLoading: waitLoading, isSuccess: waitSuccess } =
    useWaitForTransaction({
      hash: claimData?.hash,
      onSuccess(data) {
        toast("Claim success!", {
  ...
  const { isLoading: withdrawWaitLoading, isSuccess: withdrawWaitSuccess } =
    useWaitForTransaction({
  ...
  const { isLoading: depositWaitLoading, isSuccess: depositWaitSuccess } =
    useWaitForTransaction({
  ...
  const claimPrizes = async () => {
    ...
    try {
      let claimParams = processClaimParameters(
        poolerAddress,
        claimable,
        chain.name,
        currentDrawId
            ...
            <span>
              ...
              {object.polygon > 0 && (
                <span>
                  ...
                  />
                  <img
                    src="./images/ptausdc.png"
                    className="icon child child2 token-right"
                    alt="PTaUSDC"
                  />
                  &nbsp;{separator(object.polygon)} &nbsp;&nbsp;&nbsp;
                </span>
              )}
              {object.ethereum > 0 && (
                <span>
              ...
              {addressValue == address && (
                <span>
                  ...
                  >
                    <span className="actionButton">DEPOSIT</span>{" "}
                  </span>
                  {object.polygon +
                    object.ethereum +
                    object.optimism +
                    ...
                    >
                      <span className="actionButton">WITHDRAW</span>
                    </span>
                  )}
              ...
              {object.polygonTwab +
                ...
                <span>
                  BOOSTS &nbsp;&nbsp;&nbsp;&nbsp;
                  {object.polygonTwab - object.polygon > 0 && (
                    <span>
                      &nbsp;&nbsp;&nbsp;{" "}
                      <img
                        src="./images/polygontoken.png"
                      ...
                      <img
                        ...
                        alt="PTaUSDC"
                      />
                      &nbsp;+{separator(object.polygonTwab - object.polygon)}{" "}
                      &nbsp;&nbsp;&nbsp;
                    </span>
                  )}
                  {object.ethereumTwab - object.ethereum > 0 && (
                    <span>
                      {" "}
                      &nbsp;&nbsp;&nbsp;
                      <img
                      ...
                      <img
                        ...
                        alt="PTaUSDC"
                      />
                      &nbsp;+{separator(object.ethereumTwab - object.ethereum)}
                      &nbsp;&nbsp;&nbsp;
                    </span>
                  )}
                  {object.optimismTwab - object.optimism > 0 && (
                    <span>
                      {" "}
                      &nbsp;&nbsp;&nbsp;
                      <img
                      ...
                      <img
                        ...
                        alt="PTaUSDC"
                      />
                      &nbsp;+{separator(object.optimismTwab - object.optimism)}
                      &nbsp;&nbsp;&nbsp;
                    </span>
                  )}
                  {object.avalancheTwab - object.avalanche > 0 && (
                    <span>
                      {" "}
                      &nbsp;&nbsp;&nbsp;
                      <img
                      ...
                      />
                      &nbsp;+
                      {separator(object.avalancheTwab - object.avalanche)}
                    </span>
                  )}
                </span>
              )}
  ...
  const depositTo = () => {
    try {
      if (parseFloat(inputAmount) > walletBalance(usdcBalances, chain.name)) {
        setWalletMessage("insufficient balance");
      } else if (parseFloat(inputAmount) < 2) {
        setWalletMessage("2 usdc minimum");
      } else if (
  ...
  const withdrawFrom = () => {
    try {
      ...
      if (balances[0][chain.name.toLowerCase()] === undefined) {
      } else {
        withdrawBalance = parseFloat(balances[0][chain.name.toLowerCase()]);
  ...
  useEffect(() => {
    const loadWallet = async () => {
      if (modalFocus === "wallet" && address) {
        ...
        setUsdcBalances({
          polygon: polygonUsdcBalance,
          ethereum: ethereumUsdcBalance,
          optimism: optimismUsdcBalance,
          avalancheUsdcBalance,
        });
      }
    };
        ...
        <header className="card-header">
          <p className="card-header-title">
            ...
            {!validAddress && addressValue !== "" && (
              <span className="top-bar-span">&nbsp;Invalid address</span>
            )}
            &nbsp;&nbsp;
            {addressValue === "" ? (
              <div>
                <span className="top-bar-span">
                  Input
                  <span className="hidden-mobile top-bar-span">
                    {" "}
                    Pooler's address
                  </span>
                  <span className="show-mobile top-bar-span"> Addy</span>
                </span>
              </div>
            ) : (
              ""
            ...
            {xp > 0 ? (
              <span>
                <span className="numb-purp"> {separator(xp)}</span>{" "}
                <span className="hidden-mobile">DRAWS</span> XP
              </span>
            ) : (
              ""
            )}
                ...
                <thead>
                  ...
                  {addressValue === "" ? (
                    <tr>
                      <td>
                        <img
                          src="./images/yolo_nolo.png"
                          className="cool-pooly"
                        />
                      </td>
                    </tr>
                  ) : (
                          ...
                          <div className="addressText">
                            ...
                            {!item.claimed && (
                              <span>
                                ...
                                >
                                  {item.draw !== currentDrawId &&
                                  item.draw >= currentDrawId - 30 ? (
                                    <span className="claimStamp blue-hover">
                                      Claim
                                    </span>
                                  ) : (
                                    ""
                            ...
                            &nbsp;&nbsp;
                            {item.draw <= currentDrawId - 61 &&
                            !item.claimed ? (
                              <span className="stamp expired-stamp">
                                expired
                              </span>
                            ) : (
                              ""
          ...
          {modalFocus === "claim" && (
            <div>
              {isConnected && (
                <div>
                  {" "}
                  <span className="numb-purp"> {address.slice(0, 5)}</span>{" "}
                  claiming for{" "}
                  <span className="numb-purp">
                    {" "}
                    {poolerAddress.slice(0, 5)}
                  </span>
                  <br></br>
                  {filterClaimsNetworkAndExpiry(
                    claimable,
                    chain.name,
                    currentDrawId
                  ).length === 0 ? (
                    <>
                      <br></br>Switch networks, no prizes
                    </>
                  ) : (
                    <>
            ...
            <div>
              ...
              {isConnected && (
                <>
                        ...
                        <tr>
                          <td colSpan={2} style={{ textAlign: "right" }}>
                            <span className="small-balance">
                              Balance {walletBalance(usdcBalances, chain.name)}
                              {walletBalance(usdcBalances, chain.name) > 0 && (
                                <span
                                  className="max-balance"
                                  onClick={(e) =>
                                    setInputAmount(
                                      walletBalance(usdcBalances, chain.name)
                                    )
                                  }
                                >
                                  &nbsp;MAX
                  ...
                  {parseFloat(allowances[chain.name.toLowerCase()]) / 1e6 >=
                    parseFloat(inputAmount) &&
                  parseFloat(allowances[chain.name.toLowerCase()]) !== 0 ? (
                    <button
                      onClick={() => depositTo()}
                      className="myButton purple-hover"
                    >
                  ...
                  <div className="amount-container">
                    <table>
                      ...
                      <tr>
                        <td colSpan={2} style={{ textAlign: "right" }}>
                          <span className="small-balance">
                            Balance {ticketBalance(balances, chain.name)}
                            {ticketBalance(balances, chain.name) > 0 && (
                              <span
                                className="max-balance"
                                onClick={(e) =>
                                  setInputAmount(
                                    ticketBalance(balances, chain.name)
                                  )
                                }
                              >
                                &nbsp;MAX
