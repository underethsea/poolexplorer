...
function DelegationEvents(){
    ...
    const callEvents = async () => {
        ...
        const avalancheTicketAddress = "0xB27f379C050f6eD0973A01667458af6eCeBc1d90"
        const delegatedTopic = "0x4bc154dd35d6a5cb9206482ecb473cdbf2473006d6bce728b9cc0741bcc59ea2"
        const multiDelegationTopic = "0x383183291bd9a7fb8bd9c7c86c5013a89d1490c9f4e486da279804b83729a1dc"
        const multiDelegationAddress = "0x89Ee77Ce3F4C1b0346FF96E3004ff7C9f972dEF8"

        
        var endpointURL = "https://polygon-mainnet.g.alchemy.com/v2/" + process.env.REACT_APP_ALCHEMY_KEY
        ...
        "&toBlock=" +
        "latest" +
        "&address=" + polygonTicketAddress + "&topic0=" + delegatedTopic + "&apikey=" + process.env.REACT_APP_POLYGONSCAN_KEY;
        let urlFetch = await fetch(url)
        urlFetch = await urlFetch.json()
        console.log(urlFetch.result)
        console.log(url)
        let multiUrl = "https://api.polygonscan.com/api?module=logs&action=getLogs&fromBlock=" +
        fromBlock +
        "&toBlock=" +
        "latest" +
        "&address=" + multiDelegationAddress + "&topic0=" + multiDelegationTopic + "&apikey=" + process.env.REACT_APP_POLYGONSCAN_KEY;
  ...
  <div className = "card has-table has-mobile-sort-spaced">
      <header className = "card-header">
          ...
          RECENT POLYGON DELEGATIONS &nbsp;&nbsp;&nbsp;&nbsp;{popup && <div
                    className="loader"
    ...
    {claims.map((item) => 
        <tr key={item.hash}>
          <td>{TimeConvert(item.time)} {item.amount > 0 ? <img src={'./images/alligator.png'} className='emoji' alt='emoji'></img> : ""}</td>
            <td><div className="addressText">
            <a href={'https://polygonscan.com/tx/' + item.hash} target='_blank' rel="noopener noreferrer">
                                        {item.hash.substring(0,10)}
                                    </a>
                                    </div>
                                    {/* &nbsp;{checkIfYearnOrPod(item.address)} */}
                                    </td>
                                    <td>            <a href={'https://polygonscan.com/address/' + item.from} target='_blank' rel="noopener noreferrer">

                                      {item.from.substring(0,8)} </a>  {" -> "} {item.to ===  "multi" ? <><img src={'./images/usdc.png'} className='emoji' alt='emoji'></img> {item.amount} </>:  <><a href={'https://polygonscan.com/address/' + item.to} target='_blank' rel="noopener noreferrer">{item.to.substring(0,8)}</a></>}</td>
